...
def main_function(experiment_directory, continue_from, batch_split):
    ...
    arch = __import__("networks." + specs["NetworkArch"], fromlist=["Attention_SDF,Scene_MLP"])
    ...
    def save_latest(epoch,weight_dict,optimizer_dict):
        ...
        save_model(experiment_directory, "latest.pth", attention_decoder, ws.decoder_params_subdir, epoch)
    ...
    def save_checkpoints(epoch,weight_dict,optimizer_dict):
        ...
        save_model(experiment_directory, str(epoch) + ".pth", attention_decoder, ws.decoder_params_subdir,epoch)
        for scene,param in weight_dict.items():
            save_mlp_params(experiment_directory,scene,param)
        for scene,param in optimizer_dict.items():
            save_optimizer(experiment_directory,scene,param)
    ...
    optimizer_decoder = torch.optim.Adam(attention_decoder.parameters(),lr_schedules[0].get_learning_rate(0))
    ...
    if continue_from is not None:
        ...
        model_epoch = ws.load_model_parameters(
            experiment_directory, ws.decoder_params_subdir,continue_from, attention_decoder
        )
    ...
    decoder_param_num = sum(p.data.nelement() for p in attention_decoder.parameters())
